...
const deducePoints = async (userId) => {
  let params = {
    TableName: "userTable",
    Key: {
      "userId": userId
...
module.exports.redeemPoints = async ({ userId, total }) => {
  ...
  try {
    ...
    if(orderTotal > points){
      await deducePoints(userId);
      orderTotal = orderTotal - points;
      return { total: orderTotal, points }
    } else {
      throw new Error("Order total is less than the redeem points");
    }
  } catch(e) {
    throw new Error(e);
  }
...
module.exports.billCustomer = async (params) => {
  /* Bill the customer e.g using stripe tokens from the parameters, from front end, pass
  on the stripe tokens and inject into the state machine as a input parameter and use it to
  charge the customer*/
